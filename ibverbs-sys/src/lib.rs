...
unsafe fn verbs_get_ctx(ctx: *mut ibv_context) -> *mut verbs_context {
    if (*ctx).abi_compat != usize::MAX as *mut ffi::c_void {
        return ptr::null_mut();
    }
...
macro_rules! verbs_get_ctx_op {
    ($ctx: expr, $op: tt) => {{
        ...
        if vctx.is_null()
            || (*vctx).sz
                < ::core::mem::size_of::<verbs_context>() - mem::offset_of!(verbs_context, $op)
            || (*vctx).$op.is_none()
        {
            ptr::null_mut()
        } else {
...
pub unsafe fn ibv_query_device_ex(
    ...
    let ret = op(context, input, attr, size_of::<ibv_device_attr_ex>());
    if ret == libc::EOPNOTSUPP || ret == libc::ENOSYS {
        return legacy();
    }
    ret
}
...
#[inline]
pub unsafe fn ibv_req_notify_cq(cq: *mut ibv_cq, solicited_only: ffi::c_int) -> ffi::c_int {
...
#[inline]
pub unsafe fn ibv_poll_cq(cq: *mut ibv_cq, num_entries: ffi::c_int, wc: *mut ibv_wc) -> ffi::c_int {
    let ctx: *mut ibv_context = (*cq).context;
...
#[inline]
pub unsafe fn ibv_dealloc_mw(mw: *mut ibv_mw) -> ffi::c_int {
...
#[inline]
pub unsafe fn ibv_free_dm(dm: *mut ibv_dm) -> ffi::c_int {
...
#[inline]
pub unsafe fn ibv_wr_send_tso(qp: *mut ibv_qp_ex, hdr: *mut ffi::c_void, hdr_sz: u16, mss: u16) {
...
#[inline]
pub unsafe fn ibv_wr_set_inline_data(qp: *mut ibv_qp_ex, addr: *mut ffi::c_void, length: usize) {
...
#[inline]
pub unsafe fn ibv_wr_complete(qp: *mut ibv_qp_ex) -> ffi::c_int {
