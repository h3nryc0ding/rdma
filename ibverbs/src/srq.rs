...
#[derive(Clone)]
pub struct SharedReceiveQueue(sync::Arc<Owner>);

impl SharedReceiveQueue {
    pub(crate) fn ffi_ptr(&self) -> *mut ibverbs_sys::ibv_srq {
        self.0.ffi_ptr()
    }
    ...
    pub fn create(ctx: &Context, mut options: SharedReceiveQueueOptions) -> io::Result<Self> {
        ...
        let owner = unsafe {
            ...
            let srq = create_resource(
                || ibverbs_sys::ibv_create_srq_ex(context, attr),
...
struct Owner {
    srq: ptr::NonNull<ibverbs_sys::ibv_srq>,
...
impl Owner {
    fn ffi_ptr(&self) -> *mut ibverbs_sys::ibv_srq {
        self.srq.as_ptr()
    }
}
...
impl Drop for Owner {
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            let ret = ibverbs_sys::ibv_destroy_srq(srq);
            assert_eq!(ret, 0);
        }
    }
}
...
pub struct SharedReceiveQueueOptions {
    attr: ibverbs_sys::ibv_srq_init_attr_ex,
...
impl SharedReceiveQueueOptions {
    ...
    pub fn protection_domain(&mut self, pd: &ProtectionDomain) -> &mut Self {
        ...
        self.attr.comp_mask |= ibverbs_sys::IBV_SRQ_INIT_ATTR_PD;
